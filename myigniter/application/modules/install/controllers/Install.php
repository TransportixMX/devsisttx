    ...
    public function do_install()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($mi && $db && $ro) {
                ...
                $this->reset();
                $response->state = '0';
                $response->msg = 'Error can\'t set configuration on file';
                if (!$mi && !$db && !$ro) {
                    $response->msg .= 's config' . DIRECTORY_SEPARATOR . 'myigniter.php, config' . DIRECTORY_SEPARATOR . 'database.php, and config' . DIRECTORY_SEPARATOR . 'routes.php';
                    $this->output->set_content_type('application/json')->set_output(json_encode($response));
                }
                if (!$mi) {
                    $response->msg .= ' config' . DIRECTORY_SEPARATOR . 'myigniter.php';
                    $this->output->set_content_type('application/json')->set_output(json_encode($response));
                }
                if (!$db) {
                    $response->msg .= ' config' . DIRECTORY_SEPARATOR . 'database.php';
                    $this->output->set_content_type('application/json')->set_output(json_encode($response));
                }
                $response->msg .= ' config' . DIRECTORY_SEPARATOR . 'routes.php';
                $this->output->set_content_type('application/json')->set_output(json_encode($response));
            }
        }
    }
        ...
        if ($this->db->initialize()) {
            ...
            if ($this->migration->latest()) {
                ...
                if (!$mg) {
                    $this->reset();
                    $response->state = '0';
                    $response->msg = 'Failed replace file migration.php';
                    $this->output->set_content_type('application/json')->set_output(json_encode($response));
                    return;
                }
            ...
            $this->reset();
            $response->state = '0';
            $response->msg = 'Can\'t connect database';
            $this->output->set_content_type('application/json')->set_output(json_encode($response));
            return;
        }
